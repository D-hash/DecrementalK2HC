...
int main(int argc, char **argv) {
	...
	*graph = NetworKit::GraphTools::toUndirected(*graph);
    ...
    ofs << "G,V,E,K,Insertions,x,y,UTLoops,UTLengths,TotalBits,UAvgQT,UMedQT,affhubs,reached,affcycles,reachedMbfs\n";
    ...
    for(size_t t=0;t<edge_updates.size();t++){
        ...
        total_bits.push_back(kpll->total_bits);
...
//                }
    ...
    for(size_t j = 0; j < num_insertions; j++) {
        ofs << graph_location << "," << graph->numberOfNodes() << "," << graph->numberOfEdges() << "," << K << "," << j << ","
            << added_edges[j].first << "," << added_edges[j].second << ","  << update_loops_times[j] << "," << update_lengths_times[j] << "," << total_bits[j] << ","
